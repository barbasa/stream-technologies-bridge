...
case class BridgeKafkaConsumer(bridgeConfig: BridgeConfig) {
  ...
  private val groupId = bridgeConfig.conf.getOrElse("kafka.groupId", "BrokerBridgeConsumer")
  private val kafkaBootstrapServers = bridgeConfig.conf.getOrElse("kafka.bootstrapServers", "http://localhost:9092")
